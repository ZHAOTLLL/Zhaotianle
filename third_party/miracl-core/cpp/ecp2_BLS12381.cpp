...
void BLS12381::ECP2_toOctet(octet *W, ECP2 *Q, bool compress)
{
    ...
    if (alt)
    {
        ...
        if (!compress)
        {
            W->len=4*MODBYTES_B384_58;
            FP2_toBytes(&(W->val[2*MODBYTES_B384_58]), &qy);
        } else {
            W->val[0]|=0x80;
            if (FP2_islarger(&qy)==1) W->val[0]|=0x20;
            W->len=2*MODBYTES_B384_58;
        }
    } else {
        ...
        {
            W->val[0] = 0x04;
            FP2_toBytes(&(W->val[2 * MODBYTES_B384_58+1]), &qy);
            W->len = 4 * MODBYTES_B384_58 + 1;
        } else {
            W->val[0]=0x02;
            if (FP2_sign(&qy)==1) W->val[0] = 0x03;
            W->len = 2 * MODBYTES_B384_58 + 1;
        }
    }
}
/* SU= 176 */
...
int BLS12381::ECP2_fromOctet(ECP2 *Q, octet *W)
{
    ...
    if (alt)
    {
        ...
        if ((typ&0x80)==0)
        {
            FP2_fromBytes(&qy,&(W->val[2*MODBYTES_B384_58]));
        ...
        if (typ == 0x04)
        {
            FP2_fromBytes(&qy,&(W->val[2 * MODBYTES_B384_58+1]));
            if (ECP2_set(Q, &qx, &qy)) return 1;
        } else {
...
void BLS12381::ECP2_mul4(ECP2 *P, ECP2 Q[4], BIG u[4])
{
    ...
    ECP2 T[8], W;
    BIG t[4], mt;
    sign8 w[NLEN_B384_58 * BASEBITS_B384_58 + 1];
    sign8 s[NLEN_B384_58 * BASEBITS_B384_58 + 1];
