...
void BLS12381::ECP_toOctet(octet *W, ECP *P, bool compress)
{
    ...
    if (alt)
    {
        ...
        if (compress)
        {
            W->len = MODBYTES_B384_58;
            W->val[0]|=0x80;
            if (FP_islarger(&(P->y))==1) W->val[0]|=0x20;
        } else {
            W->len = 2 * MODBYTES_B384_58;
            BIG_toBytes(&(W->val[MODBYTES_B384_58]), y);
        }
    } else {
        ...
        if (compress)
        {
            ...
            BIG_toBytes(&(W->val[MODBYTES_B384_58 + 1]), y);
        }
    }
#endif
}
...
int BLS12381::ECP_fromOctet(ECP *P, octet *W)
{
    ...
    if (alt)
    {
        ...
        if ((typ&0x80)==0)
        {
            BIG_fromBytes(y, &(W->val[MODBYTES_B384_58]));
        ...
        if (typ == 0x04)
        {
            BIG_fromBytes(y, &(W->val[MODBYTES_B384_58 + 1]));
            if (ECP_set(P, x, y)) return 1;
        }
...
void BLS12381::ECP_clmul2(ECP *P, ECP *Q, BIG e, BIG f, BIG maxe)
{
    ...
    BIG te, tf, mt;
    ECP S, T, W[8], C;
    sign8 w[1 + (NLEN_B384_58 * BASEBITS_B384_58 + 1) / 2];
    int i, a, b, s, ns, nb;
