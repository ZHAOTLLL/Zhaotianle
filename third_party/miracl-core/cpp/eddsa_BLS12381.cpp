...
static void RFC7748(BIG r)
{
    int c,lg=0;
    BIG t;
    c=BLS12381::CURVE_Cof_I;
    while (c!=1)
    {
        lg++;
        c/=2;
    }
    int n=8*MODBYTES_B384_58-lg+1;
    BIG_mod2m(r,n);
...
static int encode_int(B384_58::BIG x,char *ei) {
    int b,index=0;

    if (8*MODBYTES_B384_58==MBITS_BLS12381) index=1; // extra byte needed for compression        
    b=MODBYTES_B384_58+index;
...
// encode point
static void encode(BLS12381::ECP *P,octet *W) {
    BIG x,y;
    int b,index=0;
...
static int decode_int(bool strip_sign,char *ei,B384_58::BIG x) {
    int b,index=0;
    int sign=0;
    char r[MODBYTES_B384_58+1];
...
static void decode(octet *W,BLS12381::ECP *P) {
    BIG X,Y;
    FP x,d,t,one,hint;
 ...
 */
int BLS12381::EDDSA_KEY_PAIR_GENERATE(csprng *RNG,octet* D,octet *Q)
{
    BIG r, x, y, s;
    ECP G;
    char digest[128];
...
int BLS12381::EDDSA_SIGNATURE(bool ph,octet *D, octet *context,octet *M,octet *SIG)
{
    DBIG dr;
    BIG s,sr,sd,q;
    ECP R;
    char digest[128];
    char pq[MODBYTES_B384_58+1];
...
bool BLS12381::EDDSA_VERIFY(bool ph,octet *Q,octet *context,octet *M,octet *SIG) 
{
    DBIG du;
    BIG q,t,su;
    ECP R,QD,G;
