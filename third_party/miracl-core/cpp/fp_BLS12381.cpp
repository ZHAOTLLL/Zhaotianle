...
/* So when multiplying two numbers, their product *must* be less than MODBITS_BLS12381+BASEBITS_B384_58*NLEN_B384_58 */
...
void BLS12381::FP_mod(BIG r, DBIG d)
{
    ...
    if (MConst < NEXCESS_B384_58)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B384_58 - 1];
        r[NLEN_B384_58 - 1] &= TMASK_BLS12381;
    ...
    else
    {
        ...
        r[NLEN_B384_58 - 1] &= TMASK_BLS12381;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_BLS12381) + (v << (BASEBITS_B384_58 - TBITS_BLS12381))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_BLS12381) + (v << (BASEBITS_B384_58 - TBITS_BLS12381)));
#endif
    }
    BIG_norm(r);
}
...
void BLS12381::FP_mod(BIG r, DBIG d)
{
    ...
    carry = r[NLEN_B384_58 - 1] >> TBITS_BLS12381;

    r[NLEN_B384_58 - 1] &= TMASK_BLS12381;
    r[0] += carry;

    r[224 / BASEBITS_B384_58] += carry << (224 % BASEBITS_B384_58); /* need to check that this falls mid-word */
    BIG_norm(r);
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B384_58 - 1] >> TBITS_BLS12381; // + (carry<<(BASEBITS_B384_58-TBITS_BLS12381));
    r[NLEN_B384_58 - 1] &= TMASK_BLS12381;

    r[BTset / BASEBITS_B384_58] += carry << (BTset % BASEBITS_B384_58); // need to check that this falls mid-word 
...
void BLS12381::FP_mod(BIG a, DBIG d)
{
    ...
    for (i = 0; i < NLEN_B384_58; i++)
        d[NLEN_B384_58 + i] += muladd(d[i], MConst - 1, d[i], &d[NLEN_B384_58 + i - 1]);
...
static int quo(BIG n, BIG m)
{
    ...
    if (TBITS_BLS12381 < hb)
    {
        ...
        num = (n[NLEN_B384_58 - 1] << sh) | (n[NLEN_B384_58 - 2] >> (BASEBITS_B384_58 - sh));
        den = (m[NLEN_B384_58 - 1] << sh) | (m[NLEN_B384_58 - 2] >> (BASEBITS_B384_58 - sh));
    }
    else
    {
        num = n[NLEN_B384_58 - 1];
        den = m[NLEN_B384_58 - 1];
    }
...
void BLS12381::FP_reduce(FP *a)
{
    ...
    if (a->XES > 16)
    {
        ...
        r[NLEN_B384_58 - 1] += (carry << BASEBITS_B384_58); // correction - put any carry out back in again
        BIG_sub(a->g, a->g, r);
        BIG_norm(a->g);
        sb = 2;
    }
...
{
    sign8 w[1 + (NLEN_B384_58 * BASEBITS_B384_58 + 3) / 4];
    FP tb[16];
    BIG t;
    int i, nb;
